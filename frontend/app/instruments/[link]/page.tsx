...
const InstrumentalListing = async () => {
  ...
  return (
    <div className="bg-white">
      <div className="mx-auto grid max-w-2xl grid-cols-1 items-center gap-x-8 gap-y-16 px-4 py-24 sm:px-6 sm:py-32 lg:max-w-7xl lg:grid-cols-2 lg:px-8">
        <div>
          {listings.map((instrument, index) => (
            <div key={index}>
              {Array.isArray(instrument.headertexts) &&
                instrument.headertexts.map((headerText, idx) => (
                  <div key={idx}>
        ...
        <div className="grid grid-cols-2 grid-rows-2 gap-4 sm:gap-6 lg:gap-8">
          {listings.map(
            (instrument) =>
              ...
              instrument.images.map((image, idx) => (
                <div key={idx}>
                  <Image
                    src={image.src}
                    alt={image.alt}
